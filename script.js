...
class App {
  #mapZoom = 12;
  #map;
  #startPoint = [17.026715, 51.057728];
  #endPoint;
  #workoutID = 0;
  id;
  parentID;
  ...
  _totalDistance() {
    ...
    const total = distArr.reduce(function (acc, cur, i, arr) {
      return acc + cur;
    }, 0);
    document.querySelector(".totalDist").placeholder = `${total.toFixed(2)}`;
  }
  _totalKcal() {
    const kcalArr = this.workoutsArr.map((e) => e.kcal);
    const total = kcalArr.reduce(function (acc, cur, i, arr) {
      return acc + cur;
    }, 0);
    document.querySelector(".totalKcal").placeholder = `${Math.trunc(total)}`;
  }
  _totalSpeed() {
    const speedArr = this.workoutsArr.map((e) => e.speed);
    const total = speedArr.reduce(function (acc, cur, i, arr) {
      return acc + cur;
    }, 0);
    document.querySelector(".totalSpeed").placeholder = `${total.toFixed(2)}`;
  }
  ...
  _drawTrack(e) {
    ...
    if (target.classList.contains("btn-show_track")) {
      const parent = target.closest(".workout");
      const id = parent.dataset.id?.toString().split("-")[1];
      const coords = this.trackArr[id]?.geometry.coordinates;
  ...
  _deleteWorkout(e) {
    ...
    if (target.closest(".btn-delete")) {
      ...
      const id = target.closest(".workout").dataset.id.split("-")[1];
      ...
      workoutsUpdate.forEach((e) => {
        const idArr = e.dataset.id.split("-");
  ...
  _createWorkoutMarkup(name, distance, time, speed, kcal) {
    return `
  ...
  _getRoute = async function (start, end) {
